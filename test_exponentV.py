...
def main():
    ...
    lr_init = size[1]*1e-2                  # learning rate for initial training
    lr = size[1]*0.5e-2                       # learning rate during adaptation
    ...
    exponents_V = [1.]        # V_0 ~ N(0, 1)/N^exponent_V
    exponent_W = 0.5
    do_record_data = True

    for exponent_V in exponents_V:
        # Manage save and load folders
        tag = f"new-init-exponentV{exponent_V}"  # identification of this experiment
        ...
        for seed_id, seed in enumerate(seeds):
            print(f'\n|==================================== Seed {seed} =====================================|')
            print('\n|-------------------------------- Initial training --------------------------------|')
            net0 = NonlinearDeterministicNetwork(network_size=size[1], nb_inputs=size[0], exponent_W=exponent_W, exponent_V=exponent_V,
                                                 global_mean_input_is_zero=False, do_z_score=False, rng_seed=seed_id)
            ...
            selected_wm, selected_om, wm_t_l, om_t_l = \
                net1.select_perturb(intrinsic_manifold_dim, nb_om_permuted_units=size[1]//2, nb_samples=int(1e3))
            if seed_id == 0:
