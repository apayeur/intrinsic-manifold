...
axes[0,0].set_yticks(np.arange(0.05,0.15,0.01)) #([0.05, 0.1])
...
axes[0,1].set_yticks(np.arange(1,30,2))#([1, 20])
...
for perturbation_type in ['WM', 'OM']:
    ...
    axes[1,0].set_yticks(np.arange(0,90,5))#([0, 45, 90])
...
for perturbation_type in ['WM', 'OM']:
    ...
    axes[1,1].set_yticks(np.arange(0,90,5))#[0, 45, 90])
