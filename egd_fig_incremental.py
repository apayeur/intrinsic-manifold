...
load_dir = "data/egd/incremental-training-stable-learning-many-incr"
save_fig_dir = "results/egd/incremental-training-stable-learning-many-incr"
...
m = np.mean(loss_dict['loss'], axis=0)
s = np.std(loss_dict['loss'][0], axis=0, ddof=1)
plt.plot(np.arange(m.shape[0]), m,  color=col_o, label='incr. training')
plt.fill_between(np.arange(m.shape[0]),
                 m - 2 * s / len(loss_dict['loss']) ** 0.5,
                 m + 2 * s / len(loss_dict['loss']) ** 0.5,
                 color=col_o, lw=0, alpha=0.5)
m = np.mean(loss_dict_no_incr_training['loss'], axis=0)
s = np.std(loss_dict_no_incr_training['loss'], axis=0, ddof=1)
plt.plot(np.arange(m.shape[0]), m,  ':', color=[1.3 * col_i for col_i in col_o], label='no incr. training')
plt.fill_between(np.arange(m.shape[0]),
                 m - 2 * s / len(loss_dict_no_incr_training['loss']) ** 0.5,
                 m + 2 * s / len(loss_dict_no_incr_training['loss']) ** 0.5,
