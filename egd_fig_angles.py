...
for type_of_principal_angle in types_of_principal_angle:
    ...
    fig, axes = plt.subplots(nrows=2, ncols=nb_subplots // 2 if nb_subplots % 2==0 else nb_subplots //2+1,
                           figsize=(45*units_convert['mm']*ncols, 45*units_convert['mm']*2/1.25))
    ax_dict = dict()
    axes = axes.ravel()
    ...
    for perturbation_type in ['WM', 'OM']:
        ...
        #                                  color=col_w if perturbation_type == 'WM' else col_o, alpha=0.5, lw=0)
    for angle_name in ax_dict.keys():
        ax_dict[angle_name].set_title(f'{name_mapping[angle_name]}', loc='center')
        ax_dict[angle_name].legend()
    if type_of_principal_angle == "min":
        for angle_name in ax_dict.keys():
            ax_dict[angle_name].set_ylabel(f'Smallest principal angle $[deg]$')
    else:
        for angle_name in ax_dict.keys():
            ax_dict[angle_name].set_ylabel(f'Largest principal angle $[deg]$')
    for angle_name in ax_dict.keys():
        ax_dict[angle_name].set_xlabel('Weight update post-perturbation')
