...
def main():
    ...
    lr_init = (0, 0.01, 0)
    lr_decoder = (0, 0.005, 0)
    lr_adapt = (0, 0.01, 0)  # was lr/15
    nb_iter = int(5e2)  # int(1e3)
    nb_iter_adapt = int(5e2)  # was 5e3
    seeds = np.arange(20, dtype=int)
    relearn_after_decoder_fitting = False
    do_scale_Wintuit = True

    # Manage save and load folders
    tag = f"low-rank-lr{lr_adapt[1]}-M{intrinsic_manifold_dim}-iterAdapt{nb_iter_adapt}"  # identification of this experiment, for bookkeeping
    ...
    for seed_id, seed in enumerate(seeds):
        ...
        scale = 0.95 *  if do_scale_Wintuit else 1.
