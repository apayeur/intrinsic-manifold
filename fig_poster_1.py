...
fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(45*2*units_convert['mm'], 45/1.25*units_convert['mm']), sharex=True)
# Panel A
...
#     axes[0].plot(np.arange(len(mean_)), mean_, '-' if perturbation_type=='WM' else '--',
#                 label=perturbation_type, color=col_w if perturbation_type=='WM' else col_o)
#     axes[0].fill_between(np.arange(len(mean_)),
#                         mean_ - 2*std_/loss_corr[perturbation_type].shape[0]**0.5,
#                         mean_ + 2 * std_ / loss_corr[perturbation_type].shape[0] ** 0.5,
#                         color=col_w if perturbation_type=='WM' else col_o, alpha=0.5, lw=0)
# axes[0].plot(np.arange(len(mean_)), 0.5 * np.ones(len(mean_)), ':', lw=0.5, color='grey')
# axes[0].set_yticks([0, 0.5])
# #axes[0].set_ylabel('Correlation component\nof the loss')
# axes[0].set_ylabel('$L_\mathsf{corr}$')
# axes[0].legend()
#axes[0].set_title('A', loc='left', pad=0, weight='bold', fontsize=8)
# Panel B
for perturbation_type in ['WM', 'OM']:
    ...
    axes[0].plot(np.arange(len(m)), m, label=perturbation_type, color=col_w if perturbation_type == 'WM' else col_o)
    axes[0].fill_between(np.arange(len(m)), m - 2 * sem, m + 2 * sem,
                     color=col_w if perturbation_type == 'WM' else col_o, alpha=0.5, lw=0)
axes[0].set_xlabel('Weight update post-perturbation')
axes[0].set_ylabel('Manifold overlap')
#axes[1,0].set_xticks([0, 2000, 4000])
axes[0].set_yticks([0.9, 0.95, 1])
#axes[0].set_title('B', loc='left', pad=0, weight='bold', fontsize=8)
# Panel C
...
axes[1].plot(np.arange(m_wm.shape[0]), m_wm, label='WM', color=col_w, lw=0.5)
axes[1].plot(np.arange(m_om.shape[0]), m_om, '--', label='OM', color=col_o, lw=0.5)
axes[1].fill_between(np.arange(m_wm.shape[0]),
                 m_wm - 2*std_wm/norm_gradW['loss']['WM'].shape[0]**0.5,
                 m_wm + 2*std_wm/norm_gradW['loss']['WM'].shape[0]**0.5,
                 color=col_w, lw=0, alpha=0.5)
axes[1].fill_between(np.arange(m_om.shape[0]),
                 m_om - 2*std_om/norm_gradW['loss']['OM'].shape[0]**0.5,
                 m_om + 2*std_om/norm_gradW['loss']['OM'].shape[0]**0.5,
                 color=col_o, lw=0, alpha=0.5)
axes[1].set_ylim([0, 5])
axes[1].set_yticks(list(plt.gca().get_ylim()))
#axes[1,1].set_xticks([0, 2000, 4000])
axes[1].set_xlabel('Weight update post-perturb.')
axes[1].set_ylabel(r'$\|\nabla_W L\|_F$')
#axes[1].set_title('C', loc='left', pad=0, weight='bold', fontsize=8)
...
plt.savefig(f'{save_fig_dir}/fig_poster_1.pdf')
plt.close()
plt.show()
